//函数描述：HSI OFDM调制模块，512点fft，循环前缀长度64，其中数据子载波336个
//函数参数：OFDMin-输入符号；OFDMout-输出符号；nums-输入符号个数, ti-选择是否进行载波交织
...
void HSI_OFDMer(vector<complex<double> > &OFDMin, vector<complex<double> > &OFDMout, int ti)
{
